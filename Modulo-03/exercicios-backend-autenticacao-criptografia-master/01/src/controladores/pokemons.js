...
const cadastrarPokemon = async (req, res) => {
    ...
    if (!nome) {
        return res.res.status(400).json({ mensagem: 'O campo nome é obrigatório.' });
    }

    if (!habilidades) {
        return res.res.status(400).json({ mensagem: 'O campo habilidade é obrigatório.' });
    }

    try {
        const query = `
        insert into pokemons (usuario_id, nome, apelido, habilidade, imagem)
...
const listarPokemons = async (req, res) => {
    try {
        const { rows: pokemons } = await pool.query(
            'select id, nome, habilidade, apelido, imagem from pokemons where usuario_id = $1',
            [req.usuario.id]
        )

        for (const pokemon of pokemons) {
            pokemon.habilidade = pokemon.habilidade.split(', ')
...
const detalharPokemon = async (req, res) => {
    ...
    try {
        const { rows, rowCount } = await pool.query(
            'select  id, nome, habilidade, apelido, imagem from pokemons where id = $1 and usuario_id = $2',
            [id, req.usuario.id]
        )
